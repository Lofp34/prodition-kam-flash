(()=>{
  ...
  const poetryFolder={
    ...
    parent:'livre',
    label:'Poésie',
    desc:'Apprendre les poèmes progressivement, bloc après bloc.'
  };
  if(!D.folders.some(f=>f.id===poetryFolder.id)) D.folders.push(poetryFolder);

  D.decks=D.decks.filter(d=>d.id!=='bukowski-so-you-want-to-be-a-writer');
  D.decks.push({
  ...
  // Préserve les retours à la ligne des poèmes et de leur traduction.
